...
class QuestionParser:
    ...
    def __init__(self) -> None:
        ...
        self.patterns = {
            ...
            "choice": re.compile(r"^\s*([A-E])\)\s*(.+)$"),
            "correct_answer": re.compile(r"^\s*ANSWER:\s*([A-E,\s]+)\s*$"),
            "points": re.compile(r"^\s*POINTS:\s*(\d+(?:\.\d+)?)\s*$"),
        }
    ...
    def parse(self, text: str) -> List[Question]:
        ...
        try:
            ...
            if self._is_csv_format(text):
                self.logger.info(
                    "Detected CSV format, converting to educational format"
                )
    ...
    def _parse_question_block(self, lines: List[str]) -> Tuple[Optional[Question], int]:
        ...
        while i < len(lines):
            ...
            # Check for points indicator (POINTS: 5)
            points_match = self.patterns["points"].match(line)
            if points_match:
            ...
            if correct_match:
                correct_choice = correct_match.group(1)  # Keep uppercase
                i += 1
                lines_consumed += 1
                continue
        ...
        # Mark the correct choice(s) and determine question type
        if correct_choice:
            # Parse multiple correct answers (e.g., "A,C,D" or just "B")
            correct_letters = [
                letter.strip().upper() for letter in correct_choice.split(",")
            ]

            # If multiple correct answers, this is a multiple response question
            if len(correct_letters) > 1:
                question_type = QuestionType.MULTIPLE_RESPONSE

            for choice in choices:
                for correct_letter in correct_letters:
                    if choice.id.endswith(f"_{correct_letter.lower()}"):
                        choice.is_correct = True

        # Create the question
        if not choices and question_type in (
    ...
    def _is_csv_format(self, content: str) -> bool:
        ...
        for line in lines[:5]:  # Check first 5 lines
            ...
            if (
                line.upper().startswith("MC,")
                or line.upper().startswith("MR,")
                or line.upper().startswith("TF,")
            ):
                csv_indicators += 2
    ...
    def _convert_csv_to_educational(self, csv_content: str) -> str:
        ...
        for row_num, row in enumerate(reader, 1):
            ...
            try:
                ...
                points = row[2].strip() if len(row) > 2 else "1"
                ...
                educational_lines.append(f"POINTS: {points}")
                ...
                try:
                    if question_type == "MC":  # Multiple choice
                    ...
                    elif question_type == "MR":  # Multiple response
                        # Parse comma-separated correct answers like "2,4" or "1,3,5"
                        correct_numbers = [
                            num.strip() for num in correct_answer.split(",")
                        ]
                        correct_letters = []
                        for num_str in correct_numbers:
                            try:
                                num = int(num_str)
                                if 1 <= num <= len(choices):
                                    correct_letters.append(chr(ord("A") + num - 1))
                            except ValueError:
                                continue

                        if correct_letters:
                            educational_lines.append(
                                f"ANSWER: {','.join(correct_letters)}"
                            )
                        else:
                            # Default to A if no valid answers found
                            educational_lines.append("ANSWER: A")
                        ...
                        # For unknown question types, try to parse as MC
                        try:
                            correct_num = int(correct_answer)
                            if 1 <= correct_num <= len(choices):
                                correct_letter = chr(ord("A") + correct_num - 1)
